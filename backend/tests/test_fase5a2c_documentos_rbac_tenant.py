"""Disposable-database tests for Phase F5A-2C: Documentos RBAC + tenant isolation.

Covers:
- GET /api/documentos/      -> documentos:ler   (tenant-filtered list)
- GET /api/documentos/{id}  -> documentos:ler   (404 cross-tenant, no leak)
- POST /api/documentos/     -> documentos:criar (tenant from session, hostile
  body ignored, parent Residente validated in the session ILPI before INSERT)
- PUT /api/documentos/{id}  -> documentos:atualizar (no tenant move,
  residente_id immutable by official decision)
- DELETE /api/documentos/.. -> BLOCKED_FOR_F5B_LOGICAL_DELETE (fail-closed,
  physical delete never runs; documentos:inativar does NOT authorize it)

Official decisions encoded here:
- residente_id is IMMUTABLE on PUT (silently ignored, link preserved).
- validade NULL is accepted; past validade does NOT auto-change situacao.
- responsavel_envio does NOT control authorship.
- Platform Superuser has zero grants Documentos.
"""
...
def documentos_db(request: pytest.FixtureRequest, tmp_path: pathlib.Path) -> pathlib.Path | str:
    if request.param == "sqlite":
        path = tmp_path / "fase5a2c-documentos.db"
        _run_migration(path)
        return path
...
def _new_user(*, exige_troca_senha: bool = False) -> m.User:
    ...
    return m.User(
        id=user_id,
        nome="Usuario Fase 5A-2C",
        email=f"fase5a2c-{user_id}@example.com",
...
def _new_institution(name: str = "ILPI Fase 5A-2C") -> m.Instituicao:
...
async def _documento_row(db: AsyncSession, doc_id: str) -> m.Documento | None:
    result = await db.execute(
        select(m.Documento)
        .where(m.Documento.id == doc_id)
        .execution_options(populate_existing=True)
    )
    return result.scalar_one_or_none()


def _documento_payload(residente_id: str, tipo: str = "RG") -> dict:
    return {"residente_id": residente_id, "tipo": tipo}


def test_documentos_rbac_tenant_endpoints(documentos_db):
    async def scenario(client: httpx.AsyncClient, db: AsyncSession):
        ilpi_a = _new_institution("ILPI A 5A-2C")
        ilpi_b = _new_institution("ILPI B 5A-2C")
        db.add_all([ilpi_a, ilpi_b])
        await db.flush()
        # Parent residentes, one per tenant.
        res_a = m.Residente(
            id=_new_id(),
            instituicao_id=ilpi_a.id,
            nome="Residente A 5A-2C",
            data_nascimento=date(1940, 5, 1),
        )
        res_b = m.Residente(
            id=_new_id(),
            instituicao_id=ilpi_b.id,
            nome="Residente B 5A-2C",
        ...
        reader = await _create_ilpi_user(
            db, ilpi_a, permissions={"documentos:ler"}, profile_key="leitor_doc"
        )
        writer = await _create_ilpi_user(
            db,
            ilpi_a,
            permissions={"documentos:ler", "documentos:criar", "documentos:atualizar"},
            profile_key="cuidador_doc",
        )
        no_permission = await _create_ilpi_user(
            db, ilpi_a, permissions=set(), profile_key="sem_permissao_doc"
        )
        inativar_only = await _create_ilpi_user(
            db,
            ilpi_a,
            permissions={"documentos:ler", "documentos:inativar"},
            profile_key="inativador_doc",
        )
        other_tenant = await _create_ilpi_user(
            db,
            ilpi_b,
            permissions={"documentos:ler", "documentos:criar", "documentos:atualizar"},
            profile_key="cuidador_doc_b",
        )
        pending_first_access = await _create_ilpi_user(
            db,
            ilpi_a,
            permissions={"documentos:ler", "documentos:criar", "documentos:atualizar"},
            profile_key="primeiro_acesso_doc",
        ...
        # Seed: um documento em cada ILPI.
        created_a1 = await client.post(
            "/api/documentos/", headers=headers_writer, json=_documento_payload(res_a.id, "RG")
        )
        ...
        created_b1 = await client.post(
            "/api/documentos/", headers=headers_b, json=_documento_payload(res_b.id, "CPF")
        )
        assert created_b1.status_code == 201, created_b1.text
        b1_id = created_b1.json()["id"]

        # 01. sem token -> 401.
        no_token = await client.get("/api/documentos/")
        assert no_token.status_code == 401
        assert _detail_code(no_token) == AUTHENTICATION_REQUIRED

        # 02. documentos:ler lista propria ILPI -> 200.
        listed = await client.get("/api/documentos/", headers=headers_reader)
        assert listed.status_code == 200, listed.text

        # 03. sem documentos:ler -> 403 PERMISSION_DENIED.
        denied = await client.get("/api/documentos/", headers=headers_none)
        assert denied.status_code == 403
        assert _detail_code(denied) == PERMISSION_DENIED

        # 04. listagem A nao contem B (isolamento).
        ids_a = {item["id"] for item in listed.json()}
        assert a1_id in ids_a
        assert b1_id not in ids_a
        listed_b = await client.get("/api/documentos/", headers=headers_b)
        ...
        # 05. GET proprio -> 200.
        got = await client.get(f"/api/documentos/{a1_id}", headers=headers_reader)
        assert got.status_code == 200
        assert got.json()["id"] == a1_id

        # 06. GET cross-tenant -> 404 RESOURCE_NOT_FOUND (sem leak).
        cross = await client.get(f"/api/documentos/{b1_id}", headers=headers_reader)
        assert cross.status_code == 404
        assert _detail_code(cross) == RESOURCE_NOT_FOUND

        # 07. POST com documentos:criar -> 201.
        created_a2 = await client.post(
            "/api/documentos/", headers=headers_writer, json=_documento_payload(res_a.id, "CPF")
        )
        assert created_a2.status_code == 201, created_a2.text
        a2_id = created_a2.json()["id"]
        row_a2 = await _documento_row(db, a2_id)
        assert row_a2 is not None and row_a2.instituicao_id == ilpi_a.id

        # 08. POST sem permissao -> 403 PERMISSION_DENIED (nada persistido).
        before = await _count(db, m.Documento)
        forbidden_create = await client.post(
            "/api/documentos/", headers=headers_none, json=_documento_payload(res_a.id, "Nao Criar")
        )
        assert forbidden_create.status_code == 403
        assert _detail_code(forbidden_create) == PERMISSION_DENIED
        assert await _count(db, m.Documento) == before

        # 09. POST com instituicao_id hostil -> tenant final continua A.
        hostile = await client.post(
            "/api/documentos/",
            headers=headers_writer,
            json={**_documento_payload(res_a.id, "Hostil"), "instituicao_id": ilpi_b.id},
        )
        assert hostile.status_code == 201, hostile.text
        hostile_id = hostile.json()["id"]
        hostile_row = await _documento_row(db, hostile_id)
        assert hostile_row is not None
        assert hostile_row.instituicao_id == ilpi_a.id
        hostile_cross = await client.get(f"/api/documentos/{hostile_id}", headers=headers_b)
        assert hostile_cross.status_code == 404
        assert _detail_code(hostile_cross) == RESOURCE_NOT_FOUND

        # 10. POST com residente da propria ILPI -> sucesso.
        created_a3 = await client.post(
            "/api/documentos/", headers=headers_writer, json=_documento_payload(res_a.id, "CID")
        )
        assert created_a3.status_code == 201, created_a3.text

        # 11. POST com residente de outra ILPI -> 404 e nada persistido.
        before_cross = await _count(db, m.Documento)
        cross_parent = await client.post(
            "/api/documentos/", headers=headers_writer, json=_documento_payload(res_b.id, "Cross")
        )
        assert cross_parent.status_code == 404
        assert _detail_code(cross_parent) == RESOURCE_NOT_FOUND
        assert await _count(db, m.Documento) == before_cross

        # 12. POST com residente inexistente -> 404.
        before_fake = await _count(db, m.Documento)
        fake_parent = await client.post(
            "/api/documentos/", headers=headers_writer, json=_documento_payload(_new_id(), "Fake")
        )
        assert fake_parent.status_code == 404
        assert _detail_code(fake_parent) == RESOURCE_NOT_FOUND
        assert await _count(db, m.Documento) == before_fake

        # 13. PUT autorizado -> 200.
        updated = await client.put(
            f"/api/documentos/{a2_id}",
            headers=headers_writer,
            json={"tipo": "CPF Atualizado"},
        )
        assert updated.status_code == 200, updated.text
        assert updated.json()["tipo"] == "CPF Atualizado"

        # 14. PUT sem permissao -> 403 PERMISSION_DENIED.
        forbidden_update = await client.put(
            f"/api/documentos/{a2_id}",
            headers=headers_none,
            json={"tipo": "Nao Atualizar"},
        )
        assert forbidden_update.status_code == 403
        assert _detail_code(forbidden_update) == PERMISSION_DENIED

        # 15. PUT cross-tenant -> 404 RESOURCE_NOT_FOUND (alvo intacto).
        cross_update = await client.put(
            f"/api/documentos/{b1_id}",
            headers=headers_writer,
            json={"tipo": "Tentativa Cross Tenant"},
        )
        assert cross_update.status_code == 404
        assert _detail_code(cross_update) == RESOURCE_NOT_FOUND
        intact_b = await client.get(f"/api/documentos/{b1_id}", headers=headers_b)
        assert intact_b.json()["tipo"] == "CPF"

        # 16. PUT tentando trocar tenant -> tenant preservado.
        move_attempt = await client.put(
            f"/api/documentos/{a2_id}",
            headers=headers_writer,
            json={"tipo": "Moved", "instituicao_id": ilpi_b.id},
        )
        assert move_attempt.status_code == 200, move_attempt.text
        moved_row = await _documento_row(db, a2_id)
        assert moved_row is not None and moved_row.instituicao_id == ilpi_a.id
        assert moved_row.residente_id == res_a.id

        # 17. PUT tentando alterar residente_id -> vinculo original preservado.
        ...
        transfer_row = await _documento_row(db, a2_id)
        assert transfer_row is not None and transfer_row.residente_id == res_a.id
        assert transfer_row.instituicao_id == ilpi_a.id

        # 18. Primeiro acesso pendente -> 403 FIRST_PASSWORD_CHANGE_REQUIRED.
        pending = await client.get("/api/documentos/", headers=headers_pending)
        assert pending.status_code == 403
        assert _detail_code(pending) == FIRST_PASSWORD_CHANGE_REQUIRED
        pending_create = await client.post(
            "/api/documentos/", headers=headers_pending, json=_documento_payload(res_a.id, "Bloqueado")
        )
        assert pending_create.status_code == 403
        assert _detail_code(pending_create) == FIRST_PASSWORD_CHANGE_REQUIRED

        # 19. platform_superuser global -> sem acesso Documentos.
        global_list = await client.get("/api/documentos/", headers=headers_global)
        assert global_list.status_code == 403
        assert _detail_code(global_list) == PERMISSION_DENIED
        global_get = await client.get(f"/api/documentos/{a1_id}", headers=headers_global)
        assert global_get.status_code == 403
        assert _detail_code(global_get) == PERMISSION_DENIED
        global_create = await client.post(
            "/api/documentos/", headers=headers_global, json=_documento_payload(res_a.id, "Superuser")
        )
        assert global_create.status_code == 403
        assert _detail_code(global_create) == PERMISSION_DENIED

        # 20. DELETE com usuario comum -> bloqueado.
        total_before_delete = await _count(db, m.Documento)
        blocked = await client.delete(f"/api/documentos/{a1_id}", headers=headers_writer)
        assert blocked.status_code == 403, blocked.text
        assert _detail_code(blocked) in {PERMISSION_CATALOG_PENDING, PERMISSION_DENIED}

        # 21. DELETE com documentos:inativar -> NAO executa delete fisico.
        blocked_inativar = await client.delete(f"/api/documentos/{a1_id}", headers=headers_inativar)
        assert blocked_inativar.status_code == 403, blocked_inativar.text
        assert _detail_code(blocked_inativar) in {PERMISSION_CATALOG_PENDING, PERMISSION_DENIED}

        # 22. Registro continua existindo apos tentativa DELETE.
        assert await _count(db, m.Documento) == total_before_delete
        still_there = await client.get(f"/api/documentos/{a1_id}", headers=headers_reader)
        assert still_there.status_code == 200

        # 23. documentos:inativar nao concede escrita (criar/atualizar).
        inativar_create = await client.post(
            "/api/documentos/", headers=headers_inativar, json=_documento_payload(res_a.id, "Sem Criar")
        )
        assert inativar_create.status_code == 403
        assert _detail_code(inativar_create) == PERMISSION_DENIED

        # 24. validade NULL aceita.
        created_null_val = await client.post(
            "/api/documentos/",
            headers=headers_writer,
            json={**_documento_payload(res_a.id, "Sem Validade"), "validade": None},
        )
        assert created_null_val.status_code == 201, created_null_val.text
        null_val_row = await _documento_row(db, created_null_val.json()["id"])
        assert null_val_row is not None and null_val_row.validade is None

        # 25. validade no passado NAO altera automaticamente situacao.
        created_past = await client.post(
            "/api/documentos/",
            headers=headers_writer,
            json={**_documento_payload(res_a.id, "Vencido"), "validade": "2020-01-01"},
        )
        assert created_past.status_code == 201, created_past.text
        past_row = await _documento_row(db, created_past.json()["id"])
        assert past_row is not None
        assert past_row.validade == date(2020, 1, 1)
        assert past_row.situacao == "pendente"

        # 26. responsavel_envio nao controla autoria.
        created_resp = await client.post(
            "/api/documentos/",
            headers=headers_writer,
            json={**_documento_payload(res_a.id, "Resp Test"), "responsavel_envio": "Outra Pessoa"},
        )
        assert created_resp.status_code == 201, created_resp.text
        resp_row = await _documento_row(db, created_resp.json()["id"])
        assert resp_row is not None
        assert resp_row.responsavel_envio == "Outra Pessoa"

        # 27. DELETE sem token -> 401.
        delete_no_token = await client.delete(f"/api/documentos/{a1_id}")
        assert delete_no_token.status_code == 401

    asyncio.run(_with_client(documentos_db, scenario))
...
def test_outros_modulos_clinicos_permanecem_fail_closed(documentos_db):
    ...
    async def scenario(client: httpx.AsyncClient, db: AsyncSession):
        ...
        residente = m.Residente(
            id=_new_id(),
            instituicao_id=ilpi.id,
            nome="Residente FC 5A-2C",
        ...
        # Residentes segue protegido por RBAC proprio.
        ...
        # Familiares segue protegido por RBAC proprio.
        familiares_denied = await client.get("/api/familiares/", headers=headers)
        assert familiares_denied.status_code == 403
        assert _detail_code(familiares_denied) == PERMISSION_DENIED
